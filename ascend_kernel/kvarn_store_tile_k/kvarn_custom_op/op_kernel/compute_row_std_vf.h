...
static constexpr MicroAPI::CastTrait compute_row_std_vf_default_castcfg = { MicroAPI::RegLayout::ZERO, MicroAPI::SatMode::NO_SAT, MicroAPI::MaskMergeMode::ZEROING, RoundMode::CAST_RINT };

__aicore__ inline void compute_row_std_vf(__ubuf__ half* src, __ubuf__ float* col_log_scale, __ubuf__ float* row_log_scale, __ubuf__ float* dst){
    __VEC_SCOPE__
    {
        ...
        for (uint16_t row_std_idx = (uint16_t)0; row_std_idx < (uint16_t)128; row_std_idx += (uint16_t)1) {
            MicroAPI::LoadAlign<half, MicroAPI::LoadDist::DIST_UNPACK_B16>(_tmp_reg_0, src + row_std_idx * 128);
            MicroAPI::Cast<float, half, compute_row_std_vf_default_castcfg>(values[0], _tmp_reg_0, _tmp_maskreg_1);
            MicroAPI::LoadAlign<half, MicroAPI::LoadDist::DIST_UNPACK_B16>(_tmp_reg_0, src + row_std_idx * 128 + 64);
            MicroAPI::Cast<float, half, compute_row_std_vf_default_castcfg>(values[1], _tmp_reg_0, _tmp_maskreg_1);
